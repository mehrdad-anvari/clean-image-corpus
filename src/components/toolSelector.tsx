...
import { resetSelectedAnnotation, resetSelectedVertex, setSelectedTool } from "@/features/tools/canvas";
        ...
        <div className="absolute top-4 left-4 z-20 flex gap-2">
            {tools.map(({ id, icon, label }) => (
                <button
                    key={id}
                    title={label}
                    onClick={() => {dispatch(setSelectedTool(id)); dispatch(resetSelectedAnnotation()); dispatch(resetSelectedVertex())}}
